...
resource "datadog_monitor" "AUTOGEN_integration_test_service_PATCH-multiple_methods_APM_ERROR_RATE_THRESHOLD" {
  name               = "AUTOGEN_integration_test_service_PATCH-multiple_methods_APM_ERROR_RATE_THRESHOLD"
  type               = "query alert"
  message            = "APM_ERROR_RATE_THRESHOLD triggered."
  escalation_message = "Alert escalated"

  query = "sum(last_10m): ( sum:trace.flask.request.errors{ env:integration_test_env, service:integration_test_service, resource_name:patch_/multiple_methods }.as_count() / sum:trace.flask.request.hits{ env:integration_test_env, service:integration_test_service, resource_name:patch_/multiple_methods }.as_count() ) > 0.8"
  ...
  tags = [
    "service:integration_test_service",
    "env:integration_test_env",
    "resource_name:patch_/multiple_methods"
  ]
}
