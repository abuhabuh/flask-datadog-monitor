...
resource "datadog_monitor" "AUTOGEN_integration_test_service_GET-base_test_route_all_monitors_APM_ERROR_RATE_THRESHOLD" {
  ...
  query = "sum(last_5m):(sum:trace.flask.request.errors{env:integration_test_env,service:integration_test_service,resource_name:get_/base_test_route_all_monitors}.as_count()/sum:trace.flask.request.hits{env:integration_test_env,service:integration_test_service,resource_name:get_/base_test_route_all_monitors}.as_count())>0.1"
  ...
  tags = [
    ...
    "resource_name:get_/base_test_route_all_monitors"
  ]
}
...
resource "datadog_monitor" "AUTOGEN_integration_test_service_GET-base_test_route_all_monitors_APM_LATENCY_THRESHOLD" {
  ...
  query = "avg(last_5m):avg:trace.flask.request{env:integration_test_env,service:integration_test_service,resource_name:get_/base_test_route_all_monitors}>0.1"
